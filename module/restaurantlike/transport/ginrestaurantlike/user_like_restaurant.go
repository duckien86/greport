...
func UserLikeRestaurant(appCtx appctx.AppContext) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		storeIncLikeCount := restaurantstorage.NewSQLStore(db)
		biz := restaurantlikebiz.NewLikeRestaurantBiz(store, storeIncLikeCount)
		if err := biz.LikeRestaurant(ctx, &dataModel); err != nil {
			panic(err)
		}
